...
class PPYOLOESimOTAAssigner(BaseAssigner):
    def __init__(self,
                 center_radius=2.5,
                 candidate_topk=10,
                 iou_weight=3.0,
    ...
    def _assign(self,
        ...
        if self.use_vfl:
            gt_vfl_labels = gt_labels.unsqueeze(0).repeat(
                [num_valid, 1]).reshape([-1])
            valid_pred_scores = valid_pred_scores.unsqueeze(1).repeat(
                [1, num_gt, 1]).reshape([-1, self.num_classes])
            vfl_score = torch.zeros_like(valid_pred_scores)
            vfl_score[torch.arange(0, vfl_score.shape[0],
                                   device=decoded_bboxes.device),
                      gt_vfl_labels
                      ] = pairwise_ious.view(-1)
            losses_vfl = varifocal_loss(
                valid_pred_scores, vfl_score
                ).sum(1).reshape([num_valid, num_gt])
            del gt_vfl_labels, valid_pred_scores, vfl_score
    ...
    def get_in_gt_and_in_center_info(self, priors, gt_bboxes):
        ...
        repeated_x = priors[:, 0].unsqueeze(1).expand(num_priors, num_gt)
        repeated_y = priors[:, 1].unsqueeze(1).expand(num_priors, num_gt)
        ...
        repeated_stride_x = priors[:, 2].unsqueeze(1)
        repeated_stride_y = priors[:, 3].unsqueeze(1)
        cr_x = (self.center_radius * repeated_stride_x).expand(num_priors, num_gt)
        cr_y = (self.center_radius * repeated_stride_y).expand(num_priors, num_gt)
        ct_box_l = gt_cxs - cr_x
        ct_box_t = gt_cys - cr_y
        ct_box_r = gt_cxs + cr_x
        ct_box_b = gt_cys + cr_y
    ...
    def dynamic_k_matching(self, cost, pairwise_ious, num_gt, valid_mask):
        ...
        # https://www.yuque.com/yerunyuan/ar9831/tsm0id#ejXDY
        if num_gt > 3:
            matching_matrix = cost.new_zeros(cost.shape[::-1], dtype=torch.uint8)
            ...
            if min_k != max_k:
                ...
                masks = (torch.arange(0, max_k, dtype=dynamic_ks.dtype,
                                      device=device)[None].expand(
                                          num_gt, max_k) < dynamic_ks[:, None])
                _, pos_idxes = torch.topk(cost.t(), k=max_k, dim=1, largest=False)
            ...
            matching_matrix = matching_matrix.t().contiguous()
        else:
